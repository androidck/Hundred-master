...
public class BannerImageLoader extends ImageLoader {
    ...
    public void displayImage(Context context, final Object imgObj, ImageView imageView) {
        ...
        if (imgObj != null) {
            ...
            BannerEntry entry= (BannerEntry) imgObj;
            Glide.with(context).asBitmap().load(entry.getImage()).listener(new RequestListener<Bitmap>() {
                @Override
    ...
    private void setColorList(Bitmap bitmap, String imgUrl) {
        ...
        for (int i = 0; i < colorList.size(); i++) {
            if (colorList.get(i).getImgUrl().getImage().equals(imgUrl)) {// imgUrl作为识别标志
