...
import com.community.hundred.modules.ui.setup.presenter.HistoryLoryLookPresenter;
...
@Route(path = ActivityConstant.HISTORY_LOOK)
public class HistoryLookActivity extends MyActivity<IHistoryLoryLookView, HistoryLoryLookPresenter> {
    ...
    @Override
    protected HistoryLoryLookPresenter createPresenter() {
        return new HistoryLoryLookPresenter(this);
    }
    ...
    private OnItemStateChangedListener mOnItemStateChangedListener = (viewHolder, actionState) -> {
        if (actionState == OnItemStateChangedListener.ACTION_STATE_DRAG) {
            ...
            viewHolder.itemView.setBackgroundColor(
                    ContextCompat.getColor(HistoryLookActivity.this, R.color.white));
        } else if (actionState == OnItemStateChangedListener.ACTION_STATE_IDLE) {
            // 在手松开的时候还原背景。
            ViewCompat.setBackground(viewHolder.itemView,
                    ContextCompat.getDrawable(HistoryLookActivity.this, R.color.white));
        }
    };
    ...
    private SwipeMenuCreator mSwipeMenuCreator = (swipeLeftMenu, swipeRightMenu, position) -> {
        ...
        {
            SwipeMenuItem deleteItem = new SwipeMenuItem(HistoryLookActivity.this).setBackground(
    ...
    private OnItemMenuClickListener mItemMenuClickListener = (menuBridge, position) -> {
        ...
        if (menuPosition == 0) {
            mPresenter.removeHistory(list.get(position).getId());
            adapter.notifyItemRemoved(position);
            list.remove(position);
    ...
    public void getHistoryList() {
        mPresenter.getHistoryLookList();
        mPresenter.setOnHistoryListener(list1 -> {
            list.addAll(list1);
            adapter.notifyDataSetChanged();
        });
    }
